...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        seq_pos = torch.arange(T, device=xq.device, dtype=xq.dtype).unsqueeze(1)  # Shape: (T)
        freqs = seq_pos * self.inv_freq.to(xq.dtype) # Shape: (T, dim // 2)
    ...
    def forward(self, x):
        ...
        qkv = self.c_attn(x)
        qkv = qkv.view(B, T, 3, self.n_head, C // self.n_head).transpose(2, 3)
        ...
        if self.use_flash_attn:
            q_flat = q.transpose(0, 1).reshape(-1, T, C // self.n_head)
            k_flat = k.transpose(0, 1).reshape(-1, T, C // self.n_head)
            v_flat = v.transpose(0, 1).reshape(-1, T, C // self.n_head)

            y_flat = F.scaled_dot_product_attention(
                     query=q_flat, 
                     key=k_flat, 
                     value=v_flat, 
            ...
            y = y_flat.view(self.n_head, B, T, C // self.n_head).transpose(0, 1)
        else:
    ...
    def generate(self, idx: torch.LongTensor, max_new_tokens: int, temperature:float = 1.0, do_sample:bool = False, top_k:int = None, top_p: float = 0.6):
        ...
        for _ in range(max_new_tokens):
            ...
            if not do_sample:
                ...
                _, idx_next = torch.max(logits, dim=-1, keepdim=True)
                ...
                if top_k is not None:
                    ...
                    indices_to_remove = logits < logits.topk(top_k, dim=-1).values
